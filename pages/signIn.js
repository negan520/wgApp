...
import {Container, Header, Left, Body, Right, Button, Icon, Title,Content, Form, Item, Input,Toast} from 'native-base';
...
class SignIn extends Component{
    ...
    loginSubymit=()=>{//登录提交
        ...
        signIn({"UserName":this.state.userName,"Password":this.state.passWord,"VerifyCode":"","Device":"Mobile"}).then((res)=>{
            if(res.Code=='NoError')
            {
                ...
                <Content style={{paddingLeft:10,paddingRight:15}}>
                    <Form>
                        <Item error={!/^[0-9a-zA-Z]*$/.test(this.state.userName)}>
                            ...
                            <Input style={{color:baseStyle.colorWite.color,fontSize:16}} value={this.state.userName} placeholder="请输入用户名" onChangeText={(userName) => this.setState({userName})}/>
                            {
                                this.state.userName?(<Icon onPress={()=>this.setState({userName:''})} name='close-circle'/>):(<View/>)
                            }
                        </Item>
                        <Item error={!/^[0-9a-zA-Z]*$/.test(this.state.passWord)}>
                            ...
                            <Input style={{color:baseStyle.colorWite.color,fontSize:16}} value={this.state.passWord} type="password" secureTextEntry={true} password={true} placeholder="请输入密码  " onChangeText={(passWord) => this.setState({passWord})}/>
                            {
                                this.state.passWord?(<Icon onPress={()=>{this.setState({passWord:''})}} name='close-circle'/>):(<View/>)
                            }
                        </Item>
